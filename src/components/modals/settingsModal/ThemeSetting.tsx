  ...
  return (
    <div style={merge('themeContainer')}>
      <div style={merge('themeHeader')}>
        <span style={{ ...merge('themeLabel'), ...(isMobile ? { fontSize: 38 } : {}) }}>{t('settings.theme.label')}</span>
        <span style={{ ...merge('themeHelper'), ...(isMobile ? { fontSize: 38 } : {}) }}>{t('settings.theme.helper')}</span>
      </div>
